...
test.describe('DF-3.3b Assignment-Admin-UI', () => {
    test('Happy Path inkl. Medium-Ziel, Herkunft, Activate/Deactivate und 409', async ({
        page,
    }) => {
        ...
        // Positionsfeld: Kategorie-/Werbemittel-Assignments dürfen keine Headerfelder tragen.
        ...
        await expect(page.locator('[data-test="assignment-create-link"]')).toBeVisible();
        await page.locator('[data-test="assignment-create-link"]').click();

        await expect(page.locator('[data-test="assignment-catalog-note"]')).toContainText(
            'Katalog-Admin',
        );
        await page
            .locator('[data-test="assignment-target-layer-select"]')
            .selectOption('advertising_medium');
        await expect(page.locator('[data-test="assignment-medium-select"]')).toBeVisible();
        await page.locator('[data-test="assignment-medium-select"]').selectOption({ index: 0 });
        await page
        ...
        await expect(page.locator('[data-test="assignment-status-badge"]')).toContainText(
            'Inaktiv',
        );
        ...
        await page.locator('[data-test="assignment-context-preview-button"]').click();
        await expect(
            page.locator('[data-test="assignment-context-preview"]'),
        ).toBeVisible({ timeout: 15_000 });
        await expect(
            page.locator('[data-test="assignment-context-preview-fingerprint"]'),
        ).toContainText('Fingerprint:');
        await expect(
            page.locator('[data-test="assignment-context-preview"]'),
        ).toContainText('Herkunft');
        await expect(
            page.locator('[data-test="assignment-context-preview-conflict-badge"]'),
        ).toContainText('Konfliktfrei');

        await page.locator('[data-test="assignment-activation-preview-button"]').click();
        await expect(
        ...
        await expect(page.locator('[data-test="assignment-activate-button"]')).toBeEnabled({
            timeout: 10_000,
        });
        ...
        await expect(
            page.locator('[data-test="assignment-active-lock-note"]'),
        ).toBeVisible();
    ...
    test('Konflikt blockiert Activate; Nicht-Admin erhält 403', async ({ page }) => {
        await login(page, 'admin@example.com');
        const suffix = Date.now().toString().slice(-6);
        ...
        expect(
            createA.ok(),
            `createA failed: ${createA.status()} ${await createA.text()}`,
        ).toBeTruthy();
        ...
        expect(
            activateA.ok(),
            `activateA failed: ${activateA.status()} ${await activateA.text()}`,
        ).toBeTruthy();
        ...
        expect(
            createB.ok(),
            `createB failed: ${createB.status()} ${await createB.text()}`,
        ).toBeTruthy();
        ...
        await expect(page.locator('[data-test="assignment-activate-button"]')).toBeDisabled();
