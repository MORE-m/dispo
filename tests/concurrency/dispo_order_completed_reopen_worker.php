...
$reason = $argv[6] ?? 'Concurrency Reopen.';

if ($runDir === '' || $workerId < 0 || $orderId <= 0 || $userId <= 0 || $lockVersion <= 0) {
    fwrite(STDERR, "Usage: dispo_order_completed_reopen_worker.php <run_dir> <worker_id> <order_id> <user_id> <lock_version> [reason]\n");
    exit(1);
}
...
try {
    ...
    $service = $app->make(DispoOrderCompletedReopenService::class);

    $result = $service->reopen($order, $user, $lockVersion, $reason);
