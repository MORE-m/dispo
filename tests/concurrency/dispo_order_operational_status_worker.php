...
$userId = (int) ($argv[4] ?? 0);
$targetStatus = (string) ($argv[5] ?? '');
$lockVersion = (int) ($argv[6] ?? 0);

if ($runDir === '' || $workerId < 0 || $orderId <= 0 || $userId <= 0 || $targetStatus === '' || $lockVersion <= 0) {
    fwrite(STDERR, "Usage: dispo_order_operational_status_worker.php <run_dir> <worker_id> <order_id> <user_id> <target_status> <lock_version>\n");
    exit(1);
}
...
try {
    ...
    $service = $app->make(DispoOrderOperationalStatusService::class);
    $target = DispoOrderStatus::from($targetStatus);

    $result = $service->transition($order, $user, $lockVersion, $target, null);
