...
$reason = $argv[6] ?? 'Concurrency Storno.';

if ($runDir === '' || $workerId < 0 || $orderId <= 0 || $userId <= 0 || $lockVersion <= 0) {
    fwrite(STDERR, "Usage: dispo_order_cancellation_worker.php <run_dir> <worker_id> <order_id> <user_id> <lock_version> [reason]\n");
    exit(1);
}
...
try {
    ...
    $service = $app->make(DispoOrderCancellationService::class);

    $result = $service->cancel($order, $user, $lockVersion, $reason);
