...
if (
    ...
    || $userId <= 0
    || $lockVersion <= 0
    || ($action !== 'upload' && $action !== 'exception')
    || $payload === ''
) {
    fwrite(
        STDERR,
        "Usage: dispo_order_customer_confirmation_upload_worker.php <run_dir> <worker_id> <order_id> <user_id> <lock_version> <upload|exception> <file_path_or_reason>\n",
    );
    exit(1);
}
...
try {
    ...
    if ($action === 'upload') {
        ...
        $file = new UploadedFile(
            $payload,
            'worker-'.$workerId.'-confirmation.pdf',
            'application/pdf',
            null,
            true,
        );

        $upload = $app->make(DispoOrderUploadService::class)->uploadCustomerConfirmation(
            $order,
            $user,
            $lockVersion,
            $file,
        ...
        $updated = $app->make(DispoOrderCustomerConfirmationService::class)->update(
            $order,
            $user,
            $lockVersion,
            true,
        ...
        file_put_contents(
            $resultFile,
            'OK:exception|'.$updated->lock_version,
        );
    }
