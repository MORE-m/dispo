...
- **Product-Owner-Entscheidung:** UX-GATE-A und UX-GATE-B freigegeben;
  ...
  + **Dyn-Feld-Dateien BL-P9-01c / PO-BLP901C-1** – manuell abgenommen; Merge PR **#86** folgt)
...
- **Hinweis Stand 26.09.2026 (PO-BLP901C-1):** zusätzlich freigegeben (dieser
  ...
  Header/Position) in zentraler Uploadliste (UPL-004 **ERFÜLLT**). Manuell
  abgenommen (PR **#86**, Feature-HEAD `525faab…`); Merge folgt. **Nicht**
  freigegeben: allgemeine Kommentare,
...
- **Hinweis Stand 24.09.2026 (PO-BLP802D-1):** zusätzlich freigegeben sind
  Rechnung-per-Ende je Position, Completion-Readiness, `disposed → completed`
  und Admin-Completion-Override (STA-006 / AT-18; PR #76 manuell abgenommen).
...
- **Hinweis Stand 23.09.2026 (PO-BLP802C-1):** zusätzlich freigegeben ist ausschließlich der Kundenbestätigungs-**Ausnahmeweg ohne Upload** (UPL-001 Variante B + UPL-002), inkl. Mitfreigabe zweiter Freigeber. Uploadweg: PO-BLP901A-1.
...
**Product-Owner-Teilfreigabe (25. September 2026, UX-GATE-D / BL-P8-02e / PO-BLP802E-1):**
Ausschließlich freigegeben:

- Completed-Reopen `completed → in_progress` (nur Admin/Management, Pflichtgrund)
- Storno → `cancelled` (Disposition/Admin/Management) aus erlaubten Quellstatusen
  (nicht Draft/Freigabe-Zwischenstatus); terminal
- AT-19 automatisiert testbar; **Nachzug:** PR **#77** gemergt + manuell abgenommen
...
Ausschließlich freigegeben für diesen Slice (manuell abgenommen; Feature-HEAD
`525faab…`; Merge PR **#86** folgt):
...
- **E5:** Nutzer-Ersetzen historisiert Vorgänger (Audit `reason=replace`) –
  **≠** Admin-Archiv (löscht aktive Feldreferenz)
...
- UPL-004 **ERFÜLLT**; `BL-P9-01` **abgeschlossen**
...
Material- und Audio-Uploads über BL-P9-01b; dynamische Dispo-Datei-Felder über
BL-P9-01c.
